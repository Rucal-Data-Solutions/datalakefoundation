...
class BenchmarkSpec extends AnyFunSuite with SparkSessionTest {
  ...
  private def createBenchmarkMetadata(): Metadata = {
    ...
    new Metadata(settings, override_env)
  }
  ...
  test("Benchmark Full, Merge and Historic processing") {
    ...
    val metadata = createBenchmarkMetadata()
...
class ProcessingTests extends AnyFunSuite with SparkSessionTest {
  ...
  private def createTestMetadataSettings(testId: String = ""): StringMetadataSettings = {
    val suffix = if (testId.nonEmpty) s"_${Math.abs(testId.hashCode) % 1000000}" else ""
    val testMetadataJson = s"""
    {
      "environment": {
        "name": "DEBUG (TEST)",
      ...
      "entities": [
        ...
        {
          "id": 3,
          "name": "proctest_merge_nopart$suffix",
          "enabled": true,
          "connection": "test_connection",
          "processtype": "delta",
          "watermark": [
          ...
          "columns": [
            {
              ...
              "newname": "Administration",
              "datatype": "integer",
              "fieldroles": ["calculated", "businesskey"],
              "expression": "1000"
            },
            {
              "name": "ID",
  ...
  test("Full Load Processing") {
    ...
    val metadata = new Metadata(createTestMetadataSettings(testId), override_env)
  ...
  test("System field prefix") {
    ...
    // PK column name is derived from entity name
    val pkColumnName = s"PK_${testEntity.Name}"
  ...
  test("Check Watermark") {
    ...
    val settings = createTestMetadataSettings(testId)
  ...
  test("Merge processing should handle incremental updates correctly") {
    ...
    val metadata = new Metadata(createTestMetadataSettings(testId), override_env)
    val testEntity = metadata.getEntity(3) // Using entity 3 which has delta processtype but no partition
    val paths = testEntity.getPaths
    val ioLocations = testEntity.getOutput
  ...
  test("Processing exposes current watermark values from previous run") {
    ...
    val metadata = new Metadata(createTestMetadataSettings(testId), env)
